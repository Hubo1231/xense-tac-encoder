...
class BaseModelConfig(abc.ABC):
    ...
    `model_type` and `create`. This mirrors openpi's model config contract
    while using PyTorch tensors and modules for this repository.
    """
...
class BaseModel(nn.Module, abc.ABC):
    ...
    def compute_loss(self, batch: ImageBatch) -> dict[str, torch.Tensor]:
        """Compute the model's default training loss for an image batch."""

        raise NotImplementedError(f"{type(self).__name__} must implement compute_loss().")
